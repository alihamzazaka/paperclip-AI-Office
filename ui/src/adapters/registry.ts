...
import { copilotUIAdapter } from "./copilot";
...
export function syncExternalAdapters(
  ...
  const enabledExternalTypes = new Set(
    serverAdapters
      .filter((a) => !a.disabled && !a.overrideDisabled)
      .map((a) => a.type),
  );
  ...
  for (const { type, label } of serverAdapters) {
    ...
    const fallbackParser =
      existing?.parseStdoutLine ?? processUIAdapter.parseStdoutLine;
          ...
          loadDynamicParser(type).then((parserModule) => {
            if (parserModule) {
              registerUIAdapter({
                ...
                buildAdapterConfig:
                  existing?.buildAdapterConfig ?? buildSchemaAdapterConfig,
              });
            }
          });
        }
