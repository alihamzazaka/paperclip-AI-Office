...
async function resolveSpawnTarget(
  ...
  if (/\.(cmd|bat)$/i.test(executable)) {
    ...
    return {
      command: shell,
      // Wrap the whole command line in outer quotes so cmd.exe /c parses
      // the quoted executable path correctly. windowsVerbatimArguments tells
      // Node.js not to re-escape these already-correct quotes.
